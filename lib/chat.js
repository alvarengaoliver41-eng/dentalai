...
export async function handleChat(messages, appointments = []) {
  const config = await readConfig();
  let currentAppointments = [...appointments];

  const conversation = [
    { role: 'system', content: buildSystemPrompt(config, currentAppointments) },
    ...messages
  ];
  ...
  for (let i = 0; i < MAX_ITERATIONS; i++) {
    ...
    if (msg.tool_calls && msg.tool_calls.length > 0) {
      ...
      for (const call of msg.tool_calls) {
        ...
        const { result, appointments: updated } = executeTool(call.function.name, params, currentAppointments);
        currentAppointments = updated;
  ...
  return { response: finalResponse, actions: actionsRun, appointments: currentAppointments };
}
